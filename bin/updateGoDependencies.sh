...
# dependency reference without the need to authenticat and download
...
function main {
    ...
    go get golang.org/dl/go"${GO_MOD_VERSION}"
    go install golang.org/dl/go"${GO_MOD_VERSION}@latest"
    "${GO_PATH}go${GO_MOD_VERSION}" download
...
function jenkins_git_setup() {
    ...
    go env -w GOPRIVATE=github.cyberng.com/conjur-enterprise/*
...
function local_git_setup() {
    ...
    git config --global url."git@github.cyberng.com:".insteadOf "https://github.cyberng.com/"
    go env -w GOPRIVATE=github.cyberng.com/conjur-enterprise/*
